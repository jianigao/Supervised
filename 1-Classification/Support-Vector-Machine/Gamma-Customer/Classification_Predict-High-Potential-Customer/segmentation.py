...
class Segmentation():
    ...
    def get_rfm_metric(self):
        ...
        self.transaction['time'] = pd.to_datetime(self.transaction['time']).apply(lambda x: x.replace(tzinfo=None))
        today = datetime.now()
    ...
    def create_features(self):
        ...
        user_data = raw_data.copy()

        #convert time in datetime
        user_data['time'] = pd.to_datetime(user_data['time']).apply(lambda x: x.replace(tzinfo=None))

        #convert amount into dollar
        user_data['amount_in_dollars'] = user_data['amount_in_cents']/100
        today = datetime.now()
        user_data['year'] = user_data['time'].apply(lambda x: x.year)
        user_data['month'] = user_data['time'].apply(lambda x: x.month)
        user_data['day_since'] = user_data['time'].apply(lambda x: (today-x).days)
        dataset_year = user_data.groupby('user').agg({'year':['min','max'], 
                                                      'amount_in_dollars':['sum','mean','count','max'],
                                                      'day_since':['max'], 
        ...
        dataset_mamt = user_data.groupby(['user', 'month']).agg({'amount_in_dollars': ['sum']}).reset_index()
        ...
        user_year = user_data.groupby(['user', 'year'])['amount_in_dollars'].sum().reset_index()
